...
// newTestClientWithConnID creates a Client with both userID and connID set.
// conn is nil, which is fine for handler tests that only read UserID/ConnID.
...
func TestHeartbeat_HappyPath(t *testing.T) {
	store := NewMemoryConnectionStore(0)
	...
	now := time.Now()
	err := store.Add(ctx, &ConnectionInfo{
		ID:        connID,
		UserID:    userID,
		SessionID: "sess-1",
		CreatedAt: now,
	...
	// Invoke the handler.
	client := newTestClientWithConnID(userID, connID)
...
func TestHeartbeat_ConnectionExpired(t *testing.T) {
	...
	client := newTestClientWithConnID("alice", "nonexistent-conn")
...
func TestHeartbeat_RefreshFailed(t *testing.T) {
	...
	client := newTestClientWithConnID("alice", "conn-hb-03")
