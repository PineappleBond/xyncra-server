...
// ConnectionStore.
func NewHeartbeatHandler(connStore ConnectionStore) *heartbeatHandler {
...
func (h *heartbeatHandler) HandleRequest(ctx context.Context, client *Client, req *protocol.PackageDataRequest) (json.RawMessage, error) {
	...
	if err := h.connStore.Refresh(ctx, client.ConnID()); err != nil {
		if errors.Is(err, ErrConnectionNotFound) {
			return nil, fmt.Errorf("connection expired: %w", ErrConnectionNotFound)
		}
