...
// newCleanupTaskWithDeps creates a HITLCleanupTask wired to real SQLite store,
...
func newCleanupTaskWithDeps(t *testing.T, cfg HITLCleanupConfig) (
	task *HITLCleanupTask,
...
func TestHITLCleanup_ResetsAgentStatusToIdle(t *testing.T) {
	...
	task := NewHITLCleanupTask(
		HITLCleanupConfig{Interval: time.Minute, MaxAge: 1 * time.Hour, BatchSize: 100, LockTTL: 30 * time.Second},
		testStore.Conversations,
		testStore.Questions,
		newFakeDeletableStore(),
		broadcaster,
...
// ---------------------------------------------------------------------------
// B3: Questions are soft-deleted after cleanup
// ---------------------------------------------------------------------------

// TestHITLCleanup_SoftDeletesQuestions verifies that cleanupConversation
// soft-deletes questions associated with the checkpoint (D-123 step 4).
func TestHITLCleanup_SoftDeletesQuestions(t *testing.T) {
	testStore := setupTestStore(t)
	client, redisCleanup := newTestRedisClient(t)
	defer redisCleanup()
	...
	// Create a question for the checkpoint.
	require.NoError(t, testStore.Questions.Create(ctx, &model.Question{
		ID: "q-b3-1", ConversationID: "conv-b3", CheckpointID: "cp-b3",
		InterruptID: "int-1", QuestionText: "Are you sure?", Status: model.QuestionStatusPending,
	}))

	// Verify question exists before cleanup.
	count, err := testStore.Questions.CountPendingByCheckpoint(ctx, "cp-b3")
	require.NoError(t, err)
	...
	countAfter, err := testStore.Questions.CountPendingByCheckpoint(ctx, "cp-b3")
	require.NoError(t, err)
...
// TestHITLCleanup_DefaultConfigValues verifies that NewHITLCleanupTask fills
...
func TestHITLCleanup_FullCycle(t *testing.T) {
	...
	// Create questions for the stale ones.
	require.NoError(t, testStore.Questions.Create(ctx, &model.Question{
		ID: "q-full-1", ConversationID: "conv-full-1", CheckpointID: "cp-full-1",
		InterruptID: "int-1", QuestionText: "Q1?", Status: model.QuestionStatusPending,
	}))
	require.NoError(t, testStore.Questions.Create(ctx, &model.Question{
		ID: "q-full-2", ConversationID: "conv-full-2", CheckpointID: "cp-full-2",
		InterruptID: "int-2", QuestionText: "Q2?", Status: model.QuestionStatusPending,
	}))
	...
	for _, cpID := range []string{"cp-full-1", "cp-full-2"} {
		count, countErr := testStore.Questions.CountPendingByCheckpoint(ctx, cpID)
...
// when questionStore and checkpointStore are nil (D-063 nil-safe design).
