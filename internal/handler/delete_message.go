...
func NewDeleteMessageHandler(store store.StoreAPI, broker mq.Broker, logger server.Logger) *deleteMessageHandler {
	if logger == nil {
		logger = defaultLogger{}
	}
	return &deleteMessageHandler{store: store, broker: broker, logger: logger}
}
...
func (h *deleteMessageHandler) HandleRequest(ctx context.Context, client *server.Client, req *protocol.PackageDataRequest) (json.RawMessage, error) {
	...
	for _, memberID := range members {
		...
		if err != nil {
			h.logger.Error("delete_message: failed to get latest seq (skipping UserUpdate)", "userID", memberID, "error", err)
			continue
		}
		newSeq := latestSeq + 1
	...
	if err := h.store.UserUpdateStore().Create(ctx, updates); err != nil {
		...
		h.logger.Error("delete_message: failed to create UserUpdates", "error", err)
	}

	// 9. MQ broadcast to all members' online devices (fire-and-forget, D-007).
	broadcastDeleteMessageUpdates(h.broker, h.logger, recipients)
...
func broadcastDeleteMessageUpdates(broker mq.Broker, logger server.Logger, recipients []sendMessageRecipient) {
	if broker == nil {
		return
	}
	if logger == nil {
	...
	if err != nil {
		logger.Error("delete_message: failed to marshal MQ payload", "error", err)
		return
	}
	...
	if _, err := broker.Enqueue(context.Background(), task); err != nil {
		logger.Info("delete_message: MQ enqueue failed (fire-and-forget)", "error", err)
	}
}
