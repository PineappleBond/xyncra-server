...
**为什么不用 `docker-compose.e2e.yml`**：
E2E compose 不含 observability 服务（Prometheus/Grafana/Loki 等）。本测试需要完整 observability 栈，因此使用主 `docker-compose.yml` 加 `--profile observability` 参数。主 compose 的端口（8080/6060）与 E2E 端口（18080/16060）互不冲突，可与 E2E 环境并行运行。
...
> 因此 `docker compose ps` 不会显示 `(healthy)`，但服务实际是正常的。
...
flowchart TD
    ...
    Prep --> Build[🟢 docker compose build]
    Build --> Up[🟢 docker compose --profile observability up -d]
    ...
    Pass --> Cleanup[⚪ docker compose --profile observability down -v]
    Investigate --> Cleanup
    Cleanup --> End([结束])
```
...
正确访问方式示例：
```bash
docker compose exec xyncra-server wget -qO- http://127.0.0.1:6060/debug/pprof/
# 或者
...
**操作**：
```bash
LOG_FILE=$(docker compose exec -T xyncra-server \
  sh -c 'ls -t /var/log/xyncra/*.log 2>/dev/null | head -1')
echo "当前日志文件: $LOG_FILE"
docker compose exec -T xyncra-server tail -n 20 "$LOG_FILE" > /tmp/recent-logs.jsonl
...
- ✅ 5 个告警组全部加载
- ✅ 11 条告警规则全部存在
- ❌ 任一缺失 → 检查 `prometheus/alerts.yml` 是否正确挂载、YAML 语法是否合法
...
- ❌ 缺失 → 检查 `grafana/provisioning/datasources/datasources.yml`
...
| 检查项 | 命令 |
|--------|------|
| 列出日志文件 | `docker compose exec -T xyncra-server ls -la /var/log/xyncra/` |
| 查看最近 20 行 | `docker compose exec -T xyncra-server tail -n 20 /var/log/xyncra/xyncra-server.log` |
| 验证 JSON 格式 | `docker compose exec -T xyncra-server tail -n 20 /var/log/xyncra/xyncra-server.log \| python3 -c "import json,sys;[json.loads(l) for l in sys.stdin];print('OK')"` |
| 检查日志滚动配置 | `docker compose exec -T xyncra-server sh -c 'echo $XYNCRA_LOG_MAX_SIZE $XYNCRA_LOG_MAX_AGE $XYNCRA_LOG_MAX_BACKUPS'` |
...
| Phase 6.1：Prometheus target | `xyncra-server` target `health == "up"` | ✅ | 检查 `prometheus/prometheus.yml` 中 scrape job 配置 |
| Phase 6.2：告警规则 | 5 组 11 条规则全部加载 | ✅ | 检查 `prometheus/alerts.yml` YAML 语法；确认挂载路径正确 |
| Phase 6.3：Grafana 数据源 | 4 个数据源 provision | ✅ | 检查 `grafana/provisioning/datasources/datasources.yml` |
...
| 症状 | 可能原因 | 解决方法 |
|------|---------|---------|
| `/health` 返回 404 或无法连接 | server 未启动或端口冲突 | `docker compose ps xyncra-server` 检查状态；`docker compose logs xyncra-server` 看启动错误 |
...
| `/debug/pprof/` 无法连接 | 未启用 pprof 或端口未映射 | 确认 `XYNCRA_PPROF_ENABLED=true`；检查 docker-compose.yml 端口映射包含 `6060:6060` |
| 容器内无日志文件 | `XYNCRA_LOG_DIR` 未配置 | 检查 docker-compose.yml 中 `XYNCRA_LOG_DIR=/var/log/xyncra` |
| 日志不是 JSON | `XYNCRA_LOG_FORMAT` 不是 `json` | 确认环境变量；重启 server 容器 |
| Prometheus target `down` | 网络不通或 `/metrics` 未启用 | `docker compose exec prometheus wget -O- http://xyncra-server:8080/metrics \| head -n 5` 从容器内测试 |
| 告警规则未加载 | `alerts.yml` YAML 语法错 | `docker compose exec prometheus promtool check rules /etc/prometheus/alerts.yml` |
| Grafana 数据源缺失 | provisioning 文件未挂载或语法错 | 检查 `grafana/provisioning/datasources/datasources.yml`；重启 grafana 容器 |
| Loki `/ready` 长时间返回 503 | Loki 冷启动需 ~15s（ingester warm-up） | 等待 30s 后重试；检查 `docker compose logs loki` 是否有错误 |
| Pyroscope `/ready` 长时间返回 503 | Pyroscope 冷启动需 ~60s，分 3 阶段（metastore 15s → ingester 15s → segment writer 30s） | 等待 90s 后重试；`docker compose logs pyroscope` 看阶段进度；根路径 `/` 应尽早返回 200 |
| `docker compose ps` 显示 Loki/Pyroscope 没有 `(healthy)` | **预期行为** — `grafana/loki:latest` 和 `grafana/pyroscope:latest` 是 distroless 镜像（无 `/bin/sh`），容器内无法执行健康检查 | 服务本身正常，通过 `curl http://localhost:3100/ready` 和 `http://localhost:4040/ready` 验证 |
| 端口被占用 | 其他服务占用（如本地 Prometheus）| 修改 docker-compose.yml 端口映射或停掉冲突服务 |
...
> 📌 可选扩展：如需测试 agent 相关指标（`xyncra_agent_*`、`xyncra_llm_*`）在实际 LLM 调用下的行为，参考 `.env.test` 配置 Agent 并运行 TC-000 中的 Agent 测试步骤，然后重跑 Phase 3.4（error label 验证）。
