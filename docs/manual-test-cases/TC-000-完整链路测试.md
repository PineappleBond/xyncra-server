...
npm run build
npm link  # 使 xyncra-client 命令全局可用
```

确认产出：
- `xyncra-client` 命令可用（通过 `npm link`）
- 备选调用方式：`node packages/xyncra-client-cli/dist/bin/xyncra-client.js`
...
xyncra-client --help
# 预期: 显示所有子命令（与 Go Client 一致）
```

> **变量定义**：本文档中 `$CLIENT_GO` 指 `./bin/xyncra-client`，`$CLIENT_TS` 指 `xyncra-client`（TS 版本）。
