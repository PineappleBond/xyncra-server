# TC-000: 完整链路端到端测试（双版本 Client）

> **测试编号**: TC-000
> **测试类型**: 端到端全链路
> **覆盖范围**: 冒烟测试 + 会话管理 + 草稿 + Typing Indicator + 消息操作（含回复） + 同步 + Agent 基础交互 + HITL 基础流程 + 流式文本 + 幂等性 + IPC Fallback + 设备替换 + Agent 高级功能 + ReverseRPC 基础 + 函数注册 + Logs 管理 + 速率限制 + 链路追踪验证 + LLM Debug Log Filter + **1:1 复刻一致性验证（Go vs TS）**
> **环境**: Docker E2E (D-043)
...
本测试同时覆盖 **Go Client** 和 **TypeScript Client** 两个版本。TS Client 是 Go Client 的 1:1 功能复刻（TS-D-005），CLI 命令、参数、输出格式和退出码完全一致。

**测试方法**：

1. **阶段 1–16**：使用 Go Client 作为主测试路径（文档中的默认命令），在每个阶段末尾标注 TS Client 的适配要点
2. **阶段 17（1:1 复刻一致性验证）**：使用相同参数分别运行 Go 和 TS Client，对比输出、退出码和行为一致性
...
| 进程锁 | fcntl | fs-ext | D-031 |
| 查询命令实现 | 直接读本地 SQLite (D-035) | IPC → daemon → Dexie.js | D-035 / TS-D-002 |
| 查询命令离线可用 | ✅ daemon 停止仍可查询 | ❌ 需要 daemon 运行 | D-035 |
| 运行时依赖 | 无（静态编译） | Node.js >= 20 | — |

> **注意**：由于 TS Client 的查询命令需要通过 IPC 连接 daemon（数据在 daemon 进程的 IndexedDB 内存中），
> 步骤 3.6、6.7、13.3 中"daemon 停止后仍可查询"的验证在 TS 版本中预期行为不同。

**测试目标**：验证整个系统从客户端到服务器到数据库的完整数据流正确性，覆盖核心功能决策（D-006, D-008, D-012, D-013-015, D-032, D-035, D-045, D-050, D-051, D-052, D-075, D-076, D-085, D-091, D-092, D-095, D-098/099, D-115, D-116, D-117, TS-D-002, TS-D-005, TS-D-012）；验证 OpenTelemetry 分布式追踪全链路 span 正确性（D-127 Tracing 静默降级）；验证 Go/TS 两个版本 Client 的 1:1 功能一致性。
...
- TS 特有：TS-D-002 (环境无关/依赖注入), TS-D-005 (命令完全一致), TS-D-012 (--db-path 语义变更)
...
│  TS Client: $CLIENT_TS (npm link 后全局可用)                │
└─────────────────────────────────────────────────────────────┘
```
...
> **维护提醒**：如果 TS Client 代码更新（新增命令、修改参数、变更输出格式等），需同步更新本文档（特别是阶段 17 的一致性验证）。

```bash
cd /path/to/xyncra-server/demo/web
npm install
...
确认产出：
- 编译后的入口文件：`demo/web/packages/xyncra-client-cli/dist/bin/xyncra-client.js`

**调用方式（三选一）**：

```bash
# 方式 1: npm link（推荐，使命令全局可用）
cd /path/to/xyncra-server/demo/web/packages/xyncra-client-cli
npm link
xyncra-client --version

# 方式 2: 直接 node 调用（从项目根目录）
cd /path/to/xyncra-server
node demo/web/packages/xyncra-client-cli/dist/bin/xyncra-client.js --version

# 方式 3: npx 调用（从 demo/web 目录）
cd /path/to/xyncra-server/demo/web
npx xyncra-client --version
```
...
> **变量定义**：本文档中 `$CLIENT_GO` 指 `./bin/xyncra-client`（从项目根目录执行）。
> `$CLIENT_TS` 指 TS Client 的调用命令，根据上述选择的方式不同，可以是：
> - `xyncra-client`（npm link 后）
> - `node demo/web/packages/xyncra-client-cli/dist/bin/xyncra-client.js`（直接调用）
> - `npx xyncra-client`（从 demo/web 目录）
>
> **推荐**：使用 `npm link` 方式，后续命令中直接用 `xyncra-client`。
> 阶段 1–16 默认使用 Go Client（`./bin/xyncra-client`），阶段 17 同时使用两个版本进行一致性对比。

### 3.2 启动 Docker E2E 环境

```bash
# 重要: 如果服务器代码有变更（特别是 JSON tag/model 变更），必须先清理旧 volume
# 否则旧的 user_update payload（可能是 CamelCase 格式）与新客户端的 snake_case JSON tag 不匹配，
# 导致客户端 sync 后本地 DB 字段全部为空
...
> **Jaeger Badger 权限问题**: 如果 Jaeger 容器启动失败报 `mkdir /badger/data_keys: permission denied`，
> 需要先修复 volume 权限（Jaeger v1.76.0 以 UID 10001 运行）：
>
> ```bash
> docker run --rm -v deploy_jaeger-badger-e2e:/badger alpine sh -c \
>   "mkdir -p /badger/data_keys /badger/data_values && chown -R 10001:10001 /badger"
> ```
...
| `$TS_CLI_DIR` | `demo/web/packages/xyncra-client-cli` | TS Client CLI 包目录 |
...
#### 步骤 1.7: 📘 TS Client 适配要点（阶段 1）

> 以下差异在使用 TS Client 时需要注意：

| 项目 | Go Client | TS Client |
| --- | --- | --- |
| 启动命令 | `./bin/xyncra-client listen` | `xyncra-client listen` |
| `--device-id` | **必需** | 可选（默认 SHA256(hostname)[:8]） |
| 状态文件 | `xyncra.sock` + `xyncra.lock` + `xyncra.db` | `xyncra.sock` + `xyncra.lock`（无 `xyncra.db` 文件） |
| 进程锁实现 | fcntl | fs-ext |
| 验证 DB 文件 | `ls ~/.xyncra/alice/*/xyncra.db` | 不适用（TS 使用 IndexedDB 内存存储） |

```bash
# TS Client 启动示例（注意 --device-id 可省略）
xyncra-client listen \
...
# TS Client 状态文件验证（无 xyncra.db）
ls -la ~/.xyncra/alice/*/xyncra.sock
ls -la ~/.xyncra/alice/*/xyncra.lock
# 预期: sock 和 lock 文件存在，但无 xyncra.db 文件
```
...
#### 步骤 3.7: 📘 TS Client 适配要点（阶段 3）

> **关键差异**：Go Client 的草稿查询直接读本地 SQLite，daemon 停止后仍可用；
> TS Client 的草稿查询通过 IPC 连接 daemon（IndexedDB 在 daemon 进程内存中），daemon 停止后**不可用**。

```bash
# TS Client 草稿操作 — 命令格式完全一致
xyncra-client draft save \
  --user-id alice --device-id test-device-alice-ts \
  --conversation-id "$CONV_ID" \
  --content "这是一条草稿，还没发送"

xyncra-client draft get \
  --user-id alice --device-id test-device-alice-ts \
  --conversation-id "$CONV_ID"
# 预期: 输出草稿内容（与 Go Client 一致）
```

**步骤 3.6 的 TS 版本差异**：

```bash
# TS Client: daemon 停止后 draft get 不可用
...
# （与 Go Client 不同！Go Client 可直接读 SQLite）
```

**本地 DB 验证差异**：

```bash
# Go Client: 可直接查询 SQLite
sqlite3 ~/.xyncra/alice/*/xyncra.db \
  "SELECT conversation_id, content FROM drafts WHERE conversation_id = '$CONV_ID';"

# TS Client: 无法直接查询 IndexedDB（数据在 daemon 进程内存中）
# 验证方式: 通过 CLI 命令本身（draft get）确认数据正确性
# 或使用 Node.js 脚本连接 daemon IPC 查询
```
...
#### 步骤 6.8: 📘 TS Client 适配要点（阶段 6）

> **关键差异**：Go Client 的查询命令直接读本地 SQLite，daemon 停止后仍可用 (D-035)；
> TS Client 的查询命令通过 IPC 连接 daemon（IndexedDB 在 daemon 进程内存中），daemon 停止后**不可用**。

```bash
# TS Client 查询操作 — 命令格式完全一致
xyncra-client list-conversations --user-id alice --device-id test-device-alice-ts
xyncra-client get-conversation --user-id alice --device-id test-device-alice-ts -c "$CONV_ID"
xyncra-client get-messages --user-id alice --device-id test-device-alice-ts -c "$CONV_ID"
xyncra-client search-messages --user-id alice --device-id test-device-alice-ts -c "$CONV_ID" --query "Hello"
# 预期: 输出与 Go Client 一致（格式相同，数据相同）
```

**步骤 6.7 的 TS 版本差异**：

```bash
# TS Client: daemon 停止后查询命令不可用
xyncra-client kill --user-id alice --device-id test-device-alice-ts
sleep 1

xyncra-client list-conversations --user-id alice --device-id test-device-alice-ts 2>&1
echo $?
# 预期: 退出码 2，错误信息包含 "daemon not running" 或类似
# （与 Go Client 不同！这是 TS-D-002 依赖注入架构的结果）
...
# Go Client: 可直接查询 SQLite
sqlite3 ~/.xyncra/alice/*/xyncra.db \
  "SELECT id, peer_id FROM conversations WHERE id = '$CONV_ID';"

# TS Client: 无法直接查询 IndexedDB（数据在 daemon 进程内存中）
# 验证方式: 通过 CLI 命令本身（list-conversations, get-conversation 等）确认数据正确性
# 这些命令必须通过 IPC 连接 daemon 执行
```
...
#### 步骤 13.10: 📘 TS Client 适配要点（阶段 13）

> **关键差异**：
> - Go Client: daemon 停止后，`send` fallback 到 WS 短连接，查询命令直接读 SQLite
> - TS Client: daemon 停止后，`send` 同样 fallback 到 WS 短连接，但查询命令**不可用**（需 daemon 运行）

```bash
# TS Client IPC Fallback 验证 — 与 Go Client 行为一致

# 13.10.1 停止 TS daemon
xyncra-client kill --user-id alice --device-id test-device-alice-ts
sleep 1

# 13.10.2 send fallback 到 WS 短连接（与 Go Client 一致）
xyncra-client send \
  --user-id alice --device-id test-device-alice-ts \
  --server ws://localhost:18080/ws \
  --conversation-id "$CONV_ID" \
  --content "TS Fallback test message"
# 预期: 消息发送成功（通过独立 WS 连接）

# 13.10.3 查询命令不可用（与 Go Client 不同）
xyncra-client list-conversations --user-id alice --device-id test-device-alice-ts 2>&1
echo $?
# 预期: 退出码 2（daemon not running）
# Go Client: 退出码 0（直接读 SQLite）

# 13.10.4 sync-updates 不可用（与 Go Client 一致）
xyncra-client sync-updates --user-id alice --device-id test-device-alice-ts 2>&1
echo $?
# 预期: 退出码 2（daemon not running）

# 13.10.5 重启 TS daemon
xyncra-client listen \
  --user-id alice --device-id test-device-alice-ts \
  --server ws://localhost:18080/ws \
  > "$E2E_HOME/alice-ts-daemon.log" 2>&1 &
...
### 阶段 17: 1:1 复刻一致性验证（Go vs TS）

> 本阶段验证 TypeScript Client 是否是 Go Client 的 1:1 功能复刻（TS-D-005）。
> 使用相同参数分别运行两个版本，对比输出格式、退出码和行为一致性。
...
```bash
# Go Client 帮助信息
./bin/xyncra-client --help > /tmp/go-help.txt 2>&1
./bin/xyncra-client send --help >> /tmp/go-help.txt 2>&1
./bin/xyncra-client create-conversation --help >> /tmp/go-help.txt 2>&1

# TS Client 帮助信息
xyncra-client --help > /tmp/ts-help.txt 2>&1
xyncra-client send --help >> /tmp/ts-help.txt 2>&1
xyncra-client create-conversation --help >> /tmp/ts-help.txt 2>&1

# 对比子命令列表
diff <(grep -E "^  [a-z]" /tmp/go-help.txt | sort) \
     <(grep -E "^  [a-z]" /tmp/ts-help.txt | sort)
# 预期: 无差异（子命令列表完全一致）

# 对比全局 flags
diff <(grep -E "^\s+--" /tmp/go-help.txt | sort) \
     <(grep -E "^\s+--" /tmp/ts-help.txt | sort)
# 预期: 无差异（全局 flags 完全一致）
```

#### 步骤 17.3: Daemon 启动行为对比
...
xyncra-client listen \
  --user-id alice --device-id test-device-alice-ts \
  --server ws://localhost:18080/ws \
  > "$E2E_HOME/ts-daemon.log" 2>&1 &
TS_PID=$!
sleep 2

# 验证两个 daemon 都在运行
ps -p $GO_PID && ps -p $TS_PID
# 预期: 两个进程都存在

# 对比状态文件
echo "=== Go Client 状态文件 ==="
ls -la ~/.xyncra/alice/test-device-alice/
echo "=== TS Client 状态文件 ==="
ls -la ~/.xyncra/alice/test-device-alice-ts/
# 预期: Go 有 xyncra.db，TS 没有（TS-D-012）
# sock 和 lock 文件两者都有
```
...
./bin/xyncra-client create-conversation \
  --user-id alice --device-id test-device-alice \
  --server ws://localhost:18080/ws \
  --peer-id bob > /tmp/go-create.txt 2>&1
GO_CREATE_EXIT=$?

# TS Client 创建会话（使用已创建的会话，find-or-create 幂等）
xyncra-client create-conversation \
  --user-id alice --device-id test-device-alice-ts \
  --server ws://localhost:18080/ws \
  --peer-id bob > /tmp/ts-create.txt 2>&1
TS_CREATE_EXIT=$?

# 对比退出码
echo "Go exit code: $GO_CREATE_EXIT, TS exit code: $TS_CREATE_EXIT"
# 预期: 都是 0

# 对比输出格式（应包含相同的字段：ID, Peer, Type）
echo "=== Go Client 输出 ==="
cat /tmp/go-create.txt
echo "=== TS Client 输出 ==="
cat /tmp/ts-create.txt
# 预期: 输出格式一致（字段名相同，值相同）

# 提取 Conversation ID
GO_CONV_ID=$(grep "ID:" /tmp/go-create.txt | awk '{print $2}')
TS_CONV_ID=$(grep "ID:" /tmp/ts-create.txt | awk '{print $2}')
echo "Go CONV_ID: $GO_CONV_ID, TS CONV_ID: $TS_CONV_ID"
# 预期: 两个 ID 相同（find-or-create 幂等，D-011）
```
...
./bin/xyncra-client send \
  --user-id alice --device-id test-device-alice \
  --server ws://localhost:18080/ws \
  --conversation-id "$GO_CONV_ID" \
  --content "Hello from Go Client" > /tmp/go-send.txt 2>&1
GO_SEND_EXIT=$?
...
xyncra-client send \
  ...
  --server ws://localhost:18080/ws \
  --conversation-id "$GO_CONV_ID" \
  --content "Hello from TS Client" > /tmp/ts-send.txt 2>&1
TS_SEND_EXIT=$?

# 对比退出码
echo "Go exit code: $GO_SEND_EXIT, TS exit code: $TS_SEND_EXIT"
# 预期: 都是 0
...
./bin/xyncra-client list-conversations \
  --user-id alice --device-id test-device-alice > /tmp/go-list.txt 2>&1
GO_LIST_EXIT=$?

# TS Client 查询
xyncra-client list-conversations \
  --user-id alice --device-id test-device-alice-ts > /tmp/ts-list.txt 2>&1
TS_LIST_EXIT=$?

# 对比退出码
echo "Go exit code: $GO_LIST_EXIT, TS exit code: $TS_LIST_EXIT"
# 预期: 都是 0
...
echo "=== TS Client 输出 ==="
cat /tmp/ts-list.txt
# 预期: 输出格式一致（表格列名相同，数据内容相同）

# get-messages 对比
./bin/xyncra-client get-messages \
  --user-id alice --device-id test-device-alice \
  --conversation-id "$GO_CONV_ID" > /tmp/go-msgs.txt 2>&1

xyncra-client get-messages \
  --user-id alice --device-id test-device-alice-ts \
  --conversation-id "$GO_CONV_ID" > /tmp/ts-msgs.txt 2>&1

echo "=== Go get-messages ==="
cat /tmp/go-msgs.txt
echo "=== TS get-messages ==="
cat /tmp/ts-msgs.txt
# 预期: 都显示两条消息（Go Client + TS Client 发送的）
```
...
```bash
# Go Client 幂等性测试
CLIENT_MSG_ID="idempotent-test-$(date +%s)"
./bin/xyncra-client send \
  --user-id alice --device-id test-device-alice \
  --server ws://localhost:18080/ws \
  --conversation-id "$GO_CONV_ID" \
  --client-msg-id "$CLIENT_MSG_ID" \
  --content "Idempotent test Go" > /tmp/go-idem1.txt 2>&1
...
./bin/xyncra-client send \
  ...
  --conversation-id "$GO_CONV_ID" \
  --client-msg-id "$CLIENT_MSG_ID" \
  --content "Idempotent test Go" > /tmp/go-idem2.txt 2>&1

# TS Client 幂等性测试
TS_CLIENT_MSG_ID="idempotent-test-ts-$(date +%s)"
xyncra-client send \
  --user-id alice --device-id test-device-alice-ts \
  --server ws://localhost:18080/ws \
  --conversation-id "$GO_CONV_ID" \
  --client-msg-id "$TS_CLIENT_MSG_ID" \
  --content "Idempotent test TS" > /tmp/ts-idem1.txt 2>&1
...
xyncra-client send \
  ...
  --conversation-id "$GO_CONV_ID" \
  --client-msg-id "$TS_CLIENT_MSG_ID" \
  --content "Idempotent test TS" > /tmp/ts-idem2.txt 2>&1

# 验证幂等性（两次发送应返回相同的 message ID）
echo "=== Go Client 幂等性 ==="
diff <(grep "ID:" /tmp/go-idem1.txt) <(grep "ID:" /tmp/go-idem2.txt)
# 预期: 无差异（duplicate=true，返回相同 ID）

echo "=== TS Client 幂等性 ==="
diff <(grep "ID:" /tmp/ts-idem1.txt) <(grep "ID:" /tmp/ts-idem2.txt)
# 预期: 无差异（duplicate=true，返回相同 ID）
```
...
./bin/xyncra-client create-conversation \
  --user-id alice --device-id test-device-alice \
  --server ws://localhost:18080/ws \
  --peer-id alice 2>&1 >/dev/null
echo "Go create-self exit code: $?"

xyncra-client create-conversation \
  --user-id alice --device-id test-device-alice-ts \
  --server ws://localhost:18080/ws \
  --peer-id alice 2>&1 >/dev/null
echo "TS create-self exit code: $?"
# 预期: 都是 1
...
./bin/xyncra-client send \
  ...
  --server ws://localhost:18080/ws \
  --conversation-id "00000000-0000-0000-0000-000000000000" \
  --content "fail" 2>&1 >/dev/null
echo "Go send-notfound exit code: $?"
...
xyncra-client send \
  ...
  --conversation-id "00000000-0000-0000-0000-000000000000" \
  --content "fail" 2>&1 >/dev/null
echo "TS send-notfound exit code: $?"
# 预期: 都是 1

# 3. daemon 停止后执行 IPC-only 命令
./bin/xyncra-client kill --user-id alice --device-id test-device-alice
sleep 1
xyncra-client kill --user-id alice --device-id test-device-alice-ts
sleep 1
...
./bin/xyncra-client set-typing \
  --user-id alice --device-id test-device-alice \
  --conversation-id "$GO_CONV_ID" 2>&1 >/dev/null
echo "Go set-typing-no-daemon exit code: $?"

xyncra-client set-typing \
  --user-id alice --device-id test-device-alice-ts \
  --conversation-id "$GO_CONV_ID" 2>&1 >/dev/null
echo "TS set-typing-no-daemon exit code: $?"
# 预期: 都是 2 (D-042)
```

#### 步骤 17.9: 已知差异记录

> 以下差异是**设计预期**，不视为 1:1 复刻缺陷：

| 差异项 | Go Client | TS Client | 决策编号 | 说明 |
| --- | --- | --- | --- | --- |
| 本地 DB 文件格式 | SQLite 文件 | IndexedDB（内存） | TS-D-012 | TS 使用 fake-indexeddb |
| `--db-path` 语义 | 文件路径 | IndexedDB 名称 | TS-D-012 | 字符串像路径但实际是 DB 名 |
| `--device-id` 必需性 | 必需 | 可选（有默认值） | D-033 | TS 默认 SHA256(hostname)[:8] |
| 查询命令离线可用 | ✅ 直接读 SQLite | ❌ 需要 daemon | D-035 | TS 的 IndexedDB 在 daemon 进程中 |
| 进程锁实现 | fcntl | fs-ext | D-031 | 不同语言的标准实现 |
| 运行时依赖 | 无（静态编译） | Node.js >= 20 | — | 语言运行时差异 |
| Help 格式 | cobra 格式，含需求编号 | commander.js 格式，无需求编号 | — | CLI 框架差异 |
| `completion` 子命令 | ✅ 有 | ❌ 无 | — | TS 未实现 shell 补全脚本生成 |
| Standalone fallback `send` | 自动生成 client_message_id | ❌ 缺少自动生成 | — | daemon 停止后 standalone 模式 send 时 TS 不自动生成 client_message_id，导致额外报错（不影响退出码） |
| Node.js 模块警告 | 无 | `MODULE_TYPELESS_PACKAGE_JSON` stderr 警告 | — | xyncra-protocol 包缺少 `"type": "module"`，不影响功能 |

> 除上述差异外，两个版本的 CLI 命令、参数、输出格式、退出码应完全一致。

#### 步骤 17.10: 🔍 验证 Jaeger 链路追踪

```bash
sleep 5

# 验证两个 Client 的 WebSocket 连接都被追踪
check_trace "ws.connection" "xyncra.user.id" "alice"
# 预期: >= 2（Go 和 TS 各一个连接）
...
> **Go vs TS 差异说明**：TS Client 的命令格式、参数和输出与 Go 完全一致 (TS-D-005)。
> 主要差异在于本地查询命令的实现方式：Go 直接读 SQLite，TS 通过 IPC 连接 daemon。

| 命令 | 模式 (Go) | 模式 (TS) | Go 需要 daemon | TS 需要 daemon | 验证点 |
|------|-----------|-----------|----------------|----------------|--------|
| `listen` | Daemon | Daemon | — | — | sock/lock 文件创建 + 内置函数自动注册；Go 额外创建 xyncra.db |
| `create-conversation` | IPC+WS | IPC+WS | 是 | 是 | 服务器 DB + user_updates + 对方收到通知 + 幂等性 |
| `draft save/get/delete` | IPC+本地 | IPC | 是(save/delete) / 否(get) | 是（全部需要） | Go: 本地 SQLite drafts 表；TS: daemon 内存 IndexedDB |
| `set-typing` | IPC | IPC | 是 | 是 | Ephemeral Push (Seq=0)；daemon 停止退出码 2；速率限制 |
| `send` | IPC+WS | IPC+WS | 是 | 是 | 服务器 messages 表（含 reply_to）+ user_updates + Redis MQ + 客户端 rpc_logs |
| `delete-message` | IPC+WS | IPC+WS | 是 | 是 | messages.deleted_at 非空 + user_updates |
| `delete-conversation` | IPC+WS | IPC+WS | 是 | 是 | conversations.deleted_at + 级联 + user_updates |
| `restore-conversation` | IPC+WS | IPC+WS | 是 | 是 | deleted_at 清除 + 幂等性 |
| `mark-as-read` | IPC+WS | IPC+WS | 是 | 是 | last_read_message_id 更新 + MAX 语义 |
| `sync-updates` | IPC | IPC | 是 | 是 | 本地 DB 与服务器一致 + ephemeral 过滤 + sync_states 表更新 |
| `list-conversations` | 本地 | IPC | 否 | 是 | Go: 读本地 SQLite；TS: IPC→daemon→IndexedDB |
| `get-conversation` | 本地 | IPC | 否 | 是 | 同上 |
| `get-messages` | 本地 | IPC | 否 | 是 | 同上 + 分页 |
| `search-messages` | 本地 | IPC | 否 | 是 | 同上 + 通配符转义 |
| `stream-text` | IPC | IPC | 是 | 是 | Ephemeral Push (Seq=0)；chunk + done 协议；速率限制 |
| `agent-resume` | IPC | IPC | 是 | 是 | HITL 恢复；questions 表状态更新 |
| `reload-agents` | IPC | IPC | 是 | 是 | IPC-only 命令（D-076）；daemon 停止退出码 2 |
| `kill` | 系统 | 系统 | — | — | 进程退出 + 文件清理 + stale lock 检测 |
| `logs tail/search/stats/export/cleanup` | 本地 | IPC | 否 | 是 | Go: 读本地 rpc_logs/notification_logs 表；TS: 通过 IPC |

> **关键差异总结**：
> - Go Client: 查询/日志命令可直接读本地 SQLite，daemon 停止后仍可用 (D-035)
> - TS Client: 所有需要本地数据的命令都通过 IPC 连接 daemon，daemon 停止后不可用 (TS-D-002)
> - 这一差异影响步骤 3.6、6.7、13.3 的预期结果
...
| 阶段 | 判定条件 |
|------|---------|
| 环境准备 | Redis PONG, Server /health ok, Go 和 TS 二进制可执行；Jaeger `/api/services` 返回 `xyncra-server` |
...
| 阶段 3 | 草稿 save/get/delete 正确，本地 SQLite 一致，daemon 停止后 get 仍可用（Go）；TS 版本 daemon 停止后 get 不可用（设计差异） |
...
| 阶段 13 | daemon 停止后本地查询可用（Go），sync-updates 提示错误，send fallback 成功；设备替换 4001 正常；fallback WS 连接产生 `ws.connection` span |
...
| **阶段 17** | **1:1 复刻一致性验证**：子命令列表一致，全局 flags 一致，会话创建/消息发送/查询输出格式一致，退出码一致，幂等性行为一致，已知差异符合设计预期（TS-D-012, D-033, D-035） |
| 环境清理 | 所有进程退出（Go + TS daemon），文件清理完毕 |
...
| 客户端本地 DB 字段全部为空（user_id1/user_id2/conversation_id 等） | 旧 user_update payload 使用 CamelCase JSON 键名，与新客户端 snake_case JSON tag 不匹配 | 必须用 `docker compose down -v` 清理旧 volume 后重建，确保 server DB 中没有旧格式 payload |
| `upsert conversation: store: duplicate key` 在 daemon 初始 sync 中 | 服务器 DB 有多个同一会话的 user_update，FullSync 在同一事务中 upsert 导致冲突 | 清理服务器 DB 后重试，或确认是旧数据残留后忽略（后续 sync 正常） |
| TS Client standalone `send` 报 `missing required field: client_message_id` | daemon 停止后 TS standalone fallback 不自动生成 client_message_id | 已知 TS Client 差异，退出码仍为 1（与 Go 一致），不影响功能 |

---

## 11. 环境清理

### 11.1 停止 daemon（Go + TS 两个版本）
...
xyncra-client kill --user-id alice --device-id test-device-alice-ts 2>/dev/null || true
xyncra-client kill --user-id bob --device-id test-device-bob-ts 2>/dev/null || true
...
当需要测试 Agent 的真实 LLM 交互时，需要配置 `.env` 文件。
...
> 使用 `.env.example` 作为模板（提交到版本控制），仅包含占位符。
...
| **阶段 17 (1:1 复刻验证)** | ✅ | 阶段 1-16 完成（需要两个版本的运行数据） |
| 清理 | ✅ | 所有阶段完成 |
...
| **阶段 17: 1:1 复刻一致性** | ✅ / ❌ / N/A | Go vs TS: 命令/退出码/输出格式对比 |
