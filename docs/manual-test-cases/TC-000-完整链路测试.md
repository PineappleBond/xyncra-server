...
```bash
# 获取服务器容器名
CONTAINER=$(docker compose -f docker-compose.e2e.yml ps --format "{{.Name}}" | grep server | head -1)
...
### 阶段 10.8: LLM Debug Log Filter 验证

> 验证按 User/Device ID 过滤 LLM 日志的功能。
> 需要：真实 LLM API Key（同 Phase 10）+ 服务器重启权限。

**背景**：全量 LLM 日志太大，`XYNCRA_LLM_DEBUG_USERS` / `XYNCRA_LLM_DEBUG_DEVICES` 可按 user/device（OR 逻辑）过滤，仅记录匹配请求的全量（不截断）LLM 请求/响应。

#### 步骤 10.8.1: 配置 LLM Debug Log 并重启服务器

```bash
# 停止当前服务器
# 设置 LLM 日志目录 + Debug 过滤（alice 的 LLM 调用会被记录）
export XYNCRA_LLM_LOG_DIR="$E2E_HOME/llm-logs"
export XYNCRA_LLM_DEBUG_USERS="alice"

# 重启服务器（加载新配置）
./bin/xyncra-server \
  -db-driver sqlite -db-dsn "$E2E_HOME/xyncra-e2e.db" \
  -redis-addr localhost:16379 \
  -tracing-enabled \
  -tracing-endpoint localhost:14317 \
  > "$E2E_HOME/server-debug.log" 2>&1 &
SERVER_PID=$!
sleep 3

# 验证服务器启动日志包含 debug filter 信息
cat "$E2E_HOME/server-debug.log" | grep -i "LLM debug log filter" | tail -1
# 预期: 看到 "LLM debug log filter: users=[alice] devices=[]" 或类似
```
...
#### 步骤 10.8.3: 验证 LLM 日志文件存在且包含 alice 的请求

```bash
# 检查日志文件是否存在
ls -la "$E2E_HOME/llm-logs/llm-calls.log"
# 预期: 文件存在且大小 > 0

# 查看日志内容（JSONL 格式）
cat "$E2E_HOME/llm-logs/llm-calls.log" | head -5

# 验证 agent_start/request/response 阶段存在
cat "$E2E_HOME/llm-logs/llm-calls.log" | jq -r '.phase' | sort -u
# 预期: 包含 agent_start, request, response 等阶段

# 验证 agent_id 字段
cat "$E2E_HOME/llm-logs/llm-calls.log" | jq -r '.agent_id' | sort -u
# 预期: 包含 weather-bot
```

#### 步骤 10.8.4: 验证 debug 模式下内容不截断

```bash
# 如果 request phase 的消息内容 > 4096 字符，验证未被截断
# （在 E2E 测试中对话较短，可能不会触发截断，此处为验证逻辑）
CONTENT_LEN=$(cat "$E2E_HOME/llm-logs/llm-calls.log" \
  | jq -r 'select(.phase=="request") | .messages[-1].content | length' \
  | sort -n | tail -1)
echo "Max content length in debug log: $CONTENT_LEN"
# 预期: 内容完整，不包含 "...[truncated]" 后缀
```

#### 步骤 10.8.5: 验证不匹配的用户不记录（过滤测试）

```bash
# 停止服务器，清空日志
kill $SERVER_PID 2>/dev/null; wait $SERVER_PID 2>/dev/null
> "$E2E_HOME/llm-logs/llm-calls.log"

# 只设置 bob 为 debug user（alice 不再匹配）
export XYNCRA_LLM_DEBUG_USERS="bob"
...
./bin/xyncra-server \
  ...
  -tracing-enabled \
  -tracing-endpoint localhost:14317 \
  > "$E2E_HOME/server-debug2.log" 2>&1 &
SERVER_PID=$!
sleep 3
...
# 验证日志文件仍为空（alice 不匹配 bob filter，被过滤）
wc -c "$E2E_HOME/llm-logs/llm-calls.log"
# 预期: 文件大小为 0（无记录）
```
...
# 清理 LLM debug log 环境变量
unset XYNCRA_LLM_LOG_DIR
unset XYNCRA_LLM_DEBUG_USERS
...
INTERRUPT_ID=$(sqlite3 /tmp/xyncra-e2e.db \
  "SELECT id FROM questions WHERE conversation_id = '$HITL_CONV_ID' AND status = 'pending' LIMIT 1;")
...
# 预期: local_max_seq 和 latest_seq 值一致，表示所有更新已被同步
```
...
| 阶段 10.8 | `XYNCRA_LLM_DEBUG_USERS=alice` 时 alice 的 LLM 请求被记录到 llm-calls.log（包含 request/response，无截断）；切换为 `bob` 后 alice 的请求不记录（日志为空） |
