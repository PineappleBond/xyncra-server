...
> **测试编号**: TC-000
> **测试类型**: 端到端全链路
> **覆盖范围**: 冒烟测试 + 会话管理 + 草稿 + Typing Indicator + 消息操作 + 同步 + Agent 基础交互 + 流式文本 + 幂等性 + IPC Fallback + Agent 高级功能 + ReverseRPC 基础 + 函数注册 + Logs 管理
> **环境**: Docker E2E (D-043)
> **最后更新**: 2026-07-15
...
- 交互体验：D-050 (Typing Indicator / Ephemeral Push)
...
flowchart TD
    ...
    subgraph Phase4 [阶段 4: Typing Indicator]
        P4A[Alice set-typing\n发送 typing 指示器] --> P4B[Alice set-typing --stop\n停止 typing]
        P4B --> P4C[停止 daemon 后 set-typing\n验证退出码 2 D-036]
    end
    ...
    subgraph Phase6 [阶段 6: 查询验证]
        P6A[Alice list-conversations] --> P6B[Alice get-conversation -c]
    ...
    subgraph Phase7 [阶段 7: 已读标记]
        ...
        P7B --> P7C[验证 Bob get-conversation\nunread_count = 0]
        P7C --> P7D[边界测试\nclamp/D-047/全部已读]
    end
    ...
    subgraph Phase11 [阶段 11: 流式文本]
        P11A[Alice 发送 stream-text\n到 Agent 会话] --> P11B[观察 typing indicator\nEphemeral Push]
        P11B --> P11C[验证流式响应完成\nis_done broadcast]
    end
    ...
    subgraph Phase13 [阶段 13: IPC Fallback]
    ...
    subgraph Phase14 [阶段 14: Agent 高级功能]
        P14A[修改 weather-bot.md\n添加新描述] --> P14B[调用 reload_agents RPC]
        P14B --> P14C[验证 Agent 配置已更新]
        P14C --> P14D[发送空消息给 Agent]
        P14D --> P14E[验证错误消息\nD-091 输入边界]
    end
    ...
    subgraph Phase15 [阶段 15: ReverseRPC 基础]
        P15A[Alice daemon 注册函数\nsystem.register_functions] --> P15B[验证 Redis\n函数注册信息]
        P15B --> P15C[Agent 调用客户端工具\n如配置了 DynamicToolProvider]
        P15C --> P15D[验证工具调用结果]
        P15D --> P15E[Sync 机制细节验证\nephemeral 过滤/has_more]
    end
    ...
    subgraph Phase16 [阶段 16: Logs 管理]
        ...
        P16D --> P16E[logs export --format json\n导出验证]
...
#### 步骤 4.3: Daemon 停止时 set-typing 返回错误 (D-036)
...
#### 步骤 11.1: Alice 发送流式文本请求

```bash
# 注意: stream-text 是低层流式 API，需要分多次调用：
# 1. 发送流式文本块（累积内容）
# 2. 发送完成标记 (--done)
# 这不是一个 "发送请求等待响应" 的高层命令
# E2E 测试中通过 Agent 交互间接验证流式功能

# 验证流式文本功能：通过单元测试确认
go test ./internal/e2e/ -run TestStreamText -v
# 预期: 测试通过
```
...
#### 步骤 14.2: 调用 reload_agents RPC (D-076)

```bash
# 14.2 reload_agents RPC 仅通过 WebSocket 注册，无 HTTP 端点
# 验证方式：通过 E2E 单元测试确认
go test ./internal/handler/ -run TestReloadAgents -v
# 预期: 测试通过
```

#### 步骤 14.3: 验证 Agent 配置已更新
...
# reload_agents 仅通过 WebSocket 注册，无 HTTP 端点；通过 E2E 测试验证
```
...
#### 步骤 15.1: 验证 daemon 启动时自动注册函数 (D-098)
...
| `create-conversation` | IPC+WS | 是 | 服务器 DB + 对方收到通知 + 幂等性 |
| `draft save/get/delete` | IPC+本地 | 是（save/delete）/ 否（get） | 本地 SQLite drafts 表 |
| `set-typing` | IPC | 是 | Ephemeral Push (Seq=0)；daemon 停止退出码 2 |
...
| `sync-updates` | IPC | 是 | 本地 DB 与服务器一致 + ephemeral 过滤 |
| `list-conversations` | 本地 | 否 | 读本地 SQLite + 排序 |
| `get-conversation` | 本地 | 否 | 读本地 SQLite |
| `get-messages` | 本地 | 否 | 读本地 SQLite + 分页 |
| `search-messages` | 本地 | 否 | 读本地 SQLite + 通配符转义 |
| `kill` | 系统 | — | 进程退出 + 文件清理 + stale lock 检测 |
| `logs tail/search/stats/export/cleanup` | 本地 | 否 | 读本地日志文件 |
...
| 阶段 2 | 会话在服务器 DB 创建，Bob 收到通知并可见；边界测试（自建/重复/不存在用户）通过 |
| 阶段 3 | 草稿 save/get/delete 正确，本地 SQLite 一致，daemon 停止后 get 仍可用 |
| 阶段 4 | typing indicator 正常发送和清除，daemon 停止时退出码 2 |
| 阶段 5 | 消息在服务器 DB 持久化，message_id 递增，Bob 同步后可见；边界测试通过 |
| 阶段 6 | 查询命令返回正确结果，分页 limit/has_more 正确，搜索通配符转义正常 |
| 阶段 7 | 已读游标正确更新，MAX 语义不回退，clamp 和 D-047 响应正确 |
| 阶段 8 | 消息软删除正确，权限控制有效 |
| 阶段 9 | 级联软删除/恢复正确，重复 restore/delete 幂等性通过 |
| 阶段 10 | Agent 正确响应（如配置了 Agent） |
| 阶段 11 | 流式文本正确完成（如配置了 Agent） |
| 阶段 12 | client_message_id 幂等性正确，重复发送返回 duplicate=true |
| 阶段 13 | daemon 停止后本地查询可用，sync-updates 提示错误，send fallback 成功；kill --force/stale lock 正常 |
| 阶段 14 | Agent 热加载生效，空消息触发错误消息 |
| 阶段 15 | 函数注册基础设施正常，device_id 正确传递；sync ephemeral 过滤/has_more 正确 |
| 阶段 16 | logs tail/search/stats/export/cleanup 全部正常，错误过滤正确 |
| 环境清理 | 所有进程退出，文件清理完毕 |
...
| 阶段 11: 流式文本 | ✅ / ❌ / N/A | |
| 阶段 12: 消息幂等性 | ✅ / ❌ | D-006 |
| 阶段 13: IPC Fallback | ✅ / ❌ | D-032, D-039 |
| 阶段 14: Agent 高级功能 | ✅ / ❌ / N/A | D-076, D-091 |
| 阶段 15: ReverseRPC 基础 | ✅ / ❌ / N/A | D-092, D-098 |
| 阶段 16: Logs 管理 | ✅ / ❌ | |
| 环境清理 | ✅ / ❌ | |
