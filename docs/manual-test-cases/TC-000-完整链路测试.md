...
```bash
# 在 docker-compose.e2e.yml 的 xyncra-server-e2e 的 environment 中添加：
#   - XYNCRA_TRACING_DEBUG_USERS=alice
# 然后重启服务器容器（不需要 rebuild）
docker compose -f docker-compose.e2e.yml up -d xyncra-server-e2e
sleep 5

# 验证服务器容器日志包含 debug filter 信息
docker compose -f docker-compose.e2e.yml logs xyncra-server-e2e 2>&1 | grep -i "Tracing debug LLM capture" | tail -1
...
```bash
# 修改 docker-compose.e2e.yml 中的 XYNCRA_TRACING_DEBUG_USERS 为 bob
...
```bash
# 从 docker-compose.e2e.yml 中移除 XYNCRA_TRACING_DEBUG_USERS 行
# 重启服务器容器恢复正常模式
docker compose -f docker-compose.e2e.yml up -d xyncra-server-e2e
sleep 5
```
