...
#### 步骤 10.8.1: 配置 Tracing Debug Filter 并重启服务器

```bash
# 停止当前服务器
# 设置 tracing debug filter（alice 的 LLM 调用会被记录到 span event）
export XYNCRA_TRACING_DEBUG_USERS="alice"
...
cat "$E2E_HOME/server-debug.log" | grep -i "Tracing debug LLM capture" | tail -1
...
export XYNCRA_TRACING_DEBUG_USERS="bob"
...
# 清理 tracing debug 环境变量
unset XYNCRA_TRACING_DEBUG_USERS
