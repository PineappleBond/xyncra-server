...
graph TB
    ...
    CheckPeer -->|对方是 Agent<br/>Registry 匹配| AgentFlow[Agent 处理流程]
...
- 如果 receiver 是 Agent（存在于 AgentRegistry），则触发 Agent 处理
...
- `agent/` 为示例约定，非系统强制（Registry 精确匹配）
- Agent 在协议层与普通用户完全等价
- 服务端通过 `AgentRegistry.IsAgent(userID)` 判定身份（D-054）

#### 客户端改动（MVP）

客户端仅需知道对方的 userID 即可与 Agent 交互，无需专门的身份判定函数。
...
Agent 身份判定通过检查 userID 是否存在于 `AgentRegistry` 中（精确匹配）。Agent `.md` 配置文件的 `id` 字段使用完整 userID（如 `agent/weather-bot`），`agent/` 为示例约定而非系统强制。Agent 在协议层与普通用户完全等价。

### D-055: Agent 消息格式复用

Agent 的消息与普通用户消息格式完全相同。不新增 Message.Type 枚举值，不新增 Package 类型。Agent 通过注册在 `AgentRegistry` 中的 UserID 标识。

### D-056: Agent 流式输出复用 stream_text

Agent 的流式输出完全复用 `stream_text` RPC（D-051）和累积文本模式。客户端通过 broadcast payload 中的 `user_id` 判断来源。

### D-057: Agent 思考状态复用 set_typing

Agent 的思考状态使用 `set_typing` RPC（D-050）。客户端通过 `user_id` 区分 "typing" 和 "thinking" 的 UI 展示。
...
> 基于设计文档的组件依赖关系，将实现拆分为 8 个 phase。每个 phase 可独立验收，phase 间存在明确的先后依赖。

### 依赖关系总览

```mermaid
graph LR
    P1[Phase 1<br/>Agent 配置系统] --> P2[Phase 2<br/>上下文管理]
    P1 --> P3[Phase 3<br/>消息路由与 MQ]
    P1 --> P4[Phase 4<br/>Eino 封装与流式桥接]
    P2 --> P5[Phase 5<br/>AgentTaskHandler 核心编排]
    P3 --> P5
    P4 --> P5
    P5 --> P6[Phase 6<br/>客户端集成与端到端]
    P5 --> P7[Phase 7<br/>生产化加固]
    P6 --> P7
    P7 --> P8[Phase 8<br/>高级功能]
```

**关键路径**：P1 → P4 → P5 → P6（配置 → Eino 封装 → 核心编排 → 端到端验证）

> Phase 2 和 Phase 3 可以与 Phase 4 **并行开发**，因为它们之间没有直接依赖。

---

### Phase 1: Agent 配置系统

**目标**：建立 Agent 的定义、加载和注册机制，为后续所有 phase 提供基础。

**任务**：

| # | 任务 | 文件 | 说明 |
| --- | ------ | ------ | ------ |
| 1.1 | 定义 `AgentConfig` 结构体 | `internal/agent/config.go` | ID、Name、Description、Model、APIKeyEnv、BaseURL、Parameters、Context 配置、Tools 列表 |
| 1.2 | 实现单文件 Front Matter 解析器 | `internal/agent/config.go` | 解析 YAML front matter + Markdown body（system prompt） |
| 1.3 | 实现 `AgentRegistry` | `internal/agent/registry.go` | `Load()` 从 `embed.FS` 扫描加载、`Get(id)` 查询、`IsAgent(userID)` 判断、`ListAll()` |
| 1.4 | 创建 `agents/` 目录和示例配置 | `agents/weather-bot.md` | 至少一个可运行的示例 Agent |
| 1.5 | `go:embed` 集成 | `internal/agent/embed.go` | 嵌入 `agents/*.md` 到二进制 |
| 1.6 | 单元测试 | `internal/agent/registry_test.go` | 配置解析、加载、IsAgent 判断 |

**产出**：`AgentRegistry` 可加载配置并查询 Agent 信息。

**依赖**：无（第一个 phase）

**验收标准**：
- 能从 `agents/` 目录加载 Agent 配置
- `IsAgent("agent/weather-bot")` 返回 `true`
- `IsAgent("user/alice")` 返回 `false`
- 无效配置文件被跳过并记录日志
...
**目标**：实现对话历史的加载、缓存和 Token 裁剪，为 AgentTaskHandler 提供上下文数据。

**任务**：

| # | 任务 | 文件 | 说明 |
| --- | ------ | ------ | ------ |
| 2.1 | 新增 `ListRecentByConversation()` 方法 | `internal/store/message.go` | 按 conversation 查询最近 N 条消息，支持时间/ID 排序 |
| 2.2 | 定义 `ContextManager` 接口 | `internal/agent/context.go` | `GetContext(ctx, conversationID) -> []Message` |
| 2.3 | 实现 `DBContextManager` | `internal/agent/db_context_manager.go` | 从 DB 加载消息、`sync.Map` 缓存热路径、TTL 清理 |
| 2.4 | 实现 Token 裁剪逻辑 | `internal/agent/db_context_manager.go` | `tiktoken-go` 计数 + 固定消息数 fallback |
| 2.5 | 实现消息类型过滤 | `internal/agent/db_context_manager.go` | 区分 user/assistant/summary/tool_call/tool_result |
| 2.6 | 单元测试 | `internal/agent/db_context_manager_test.go` | 缓存命中/未命中、Token 裁剪、消息过滤 |

**产出**：`ContextManager` 能加载并裁剪对话上下文。

**依赖**：Phase 1（需要 `AgentConfig.Context` 配置中的 `max_tokens`、`max_messages` 参数）

**验收标准**：
- 从 DB 加载指定 conversation 的最近消息
- Token 超过 `max_tokens` 时自动裁剪最旧消息
- `sync.Map` 缓存命中时不查 DB
- tiktoken 失败时 fallback 到 `max_messages` 裁剪

---

### Phase 3: 消息路由与 MQ 集成

**目标**：在消息发送流程中识别 Agent 目标，将任务入队到 MQ。

**任务**：

| # | 任务 | 文件 | 说明 |
| --- | ------ | ------ | ------ |
| 3.1 | 新增 `TypeAgentProcess` task type | `internal/mq/mq.go` | 定义常量 + payload 结构体（MessageID、ConversationID、AgentID、SenderID） |
| 3.2 | 修改 `send_message` handler | `internal/handler/send_message.go` | 消息持久化后，检查对话对方的 UserID 是否存在于 AgentRegistry |
| 3.3 | Agent 目标检测逻辑 | `internal/handler/send_message.go` | 获取 Conversation → 找到 peer UserID → `AgentRegistry.IsAgent(peer)` → Enqueue |
| 3.4 | 非 Agent 消息保持不变 | `internal/handler/send_message.go` | peer 不是 Agent 时走原有流程，零影响 |
| 3.5 | 集成测试 | | 发送消息给 Agent UserID → 验证 MQ 中有对应 task |

**产出**：发送给 Agent 的消息自动入队 MQ。

**依赖**：Phase 1（需要 `AgentRegistry.IsAgent()` 来判断目标是否是 Agent）

**可与 Phase 2、Phase 4 并行开发**

**验收标准**：
- 发送消息给 `agent/weather-bot` → MQ 入队 `TypeAgentProcess`
- 发送消息给普通用户 → 走原有流程，不入队
- Task payload 包含完整的 MessageID、ConversationID、AgentID

---

### Phase 4: Eino 封装与流式桥接

**目标**：封装 Eino ChatModel，实现 LLM 调用和流式输出的桥接层。

**任务**：

| # | 任务 | 文件 | 说明 |
| --- | ------ | ------ | ------ |
| 4.1 | 实现 `LLMClient` 工厂 | `internal/agent/eino_agent.go` | 根据 `AgentConfig.Model` + `APIKeyEnv` 创建 Eino `ChatModel`（支持 OpenAI/Claude/Ollama） |
| 4.2 | 实现 `ChatModelAgent` 构建器 | `internal/agent/eino_agent.go` | 从 AgentConfig 构建 `adk.NewChatModelAgent`，设置 Instruction、Model、MaxIterations |
| 4.3 | 实现 `Runner` 封装 | `internal/agent/eino_agent.go` | `adk.NewRunner` + `EnableStreaming: true`，返回 `AsyncIterator[*AgentEvent]` |
| 4.4 | 实现 `StreamBridge` | `internal/agent/stream_bridge.go` | 将 Eino `StreamReader` 转为 Xyncra 的 `StreamChunk`，含 50ms 节流 |
| 4.5 | 实现 `broadcastHelper` | `internal/agent/broadcast.go` | 封装 `BroadcastUpdates` 调用，支持 streaming + is_done 标记 |
| 4.6 | 添加 `go.mod` 依赖 | `go.mod` | `github.com/cloudwego/eino`、`github.com/cloudwego/eino-ext` 相关包 |
| 4.7 | 单元测试 / Mock LLM | `internal/agent/eino_agent_test.go` | Mock ChatModel，验证流式输出管道 |

**产出**：给定 AgentConfig，可以构建 Eino Agent 并获取流式输出。

**依赖**：Phase 1（读取 AgentConfig 中的 model、api_key、parameters 等配置）

**可与 Phase 2、Phase 3 并行开发**

**验收标准**：
- 从 AgentConfig 创建对应的 ChatModel（OpenAI / Claude）
- Runner 返回流式事件迭代器
- StreamBridge 将 Eino chunk 转为 Xyncra StreamChunk
- 50ms 节流正常工作

---

### Phase 5: AgentTaskHandler 核心编排

**目标**：实现 MQ task handler，串联所有组件完成 Agent 处理的完整流程。

**任务**：

| # | 任务 | 文件 | 说明 |
| --- | ------ | ------ | ------ |
| 5.1 | 实现 `AgentTaskHandler` | `internal/agent/task_handler.go` | 消费 `TypeAgentProcess` task 的主处理器 |
| 5.2 | 处理流程编排 | `internal/agent/task_handler.go` | set_typing(true) → GetContext → BuildMessages → Runner.Query → StreamBridge → broadcastStreaming → set_typing(false) → SendMessage 持久化 → broadcast 完整消息 |
| 5.3 | 错误处理与重试 | `internal/agent/task_handler.go` | LLM 超时/限流/网络错误分类处理，利用 Asynq 重试机制 |
| 5.4 | 幂等性检查 | `internal/agent/task_handler.go` | 处理前检查 MessageID 是否已处理（Redis/DB），避免重复 |
| 5.5 | 注册 handler 到 MQ | `internal/mq/mq.go` 或 `task_handler.go` | `mq.RegisterHandler(TypeAgentProcess, handler.Handle)` |
| 5.6 | `main.go` 初始化 | `cmd/xyncra-server/main.go` | 初始化 AgentRegistry → ContextManager → LLMClient → AgentTaskHandler → 注册到 MQ |
| 5.7 | 集成测试 | | 完整流程：MQ 入队 → handler 消费 → LLM 调用 → 流式广播 → 消息持久化 |

**产出**：完整的 Agent 处理管道，从 MQ task 到流式回复。

**依赖**：Phase 1（AgentRegistry）+ Phase 2（ContextManager）+ Phase 3（MQ task type）+ Phase 4（Eino 封装 + StreamBridge）

**这是最核心的 phase，依赖前面所有 phase 的产出。**

**验收标准**：
- 发送消息给 Agent → Agent 调用 LLM → 流式广播 token → 持久化完整回复
- LLM 超时自动重试（最多 3 次）
- 重复 task 被幂等性检查跳过
- set_typing 在开始/结束时正确切换
- 错误情况（LLM 不可用、DB 异常）被妥善处理

---

### Phase 6: 客户端集成与端到端验证

**目标**：客户端侧适配 Agent 消息展示，完成端到端流程验证。

**任务**：

| # | 任务 | 文件 | 说明 |
| --- | ------ | ------ | ------ |
| 6.1 | 客户端 Agent 消息展示适配 | 客户端代码 | 根据 `user_id` 适配 Agent 消息 UI |
| 6.2 | TypingHandler 适配 | 客户端代码 | Agent 的 typing 显示为 "thinking..." 而非 "typing..." |
| 6.3 | StreamingHandler 适配 | 客户端代码 | Agent 的 streaming 消息正常累积显示（已有逻辑，仅需验证） |
| 6.4 | Agent 消息的 UI 区分 | 客户端代码 | 可选：Agent 消息头像/标识与普通用户不同 |
| 6.5 | 端到端测试 | | 真实场景：用户发消息 → Agent 思考 → 流式回复 → 完整消息 |
| 6.6 | 多轮对话测试 | | 连续发送多条消息，验证上下文保持 |

**产出**：客户端可正确展示 Agent 的思考和流式回复。

**依赖**：Phase 5（需要完整的后端处理管道）

**验收标准**：
- 客户端收到 Agent 流式 token 并实时显示
- typing 状态正确展示（thinking vs typing）
- 多轮对话中 Agent 能引用之前的对话内容
- 完整消息在流式结束后正确持久化并可回看
...
**目标**：优化性能、添加监控、提升可靠性，使系统达到生产可用状态。

**任务**：

| # | 任务 | 文件 | 说明 |
| --- | ------ | ------ | ------ |
| 7.1 | per-conversation worker 串行队列 | `internal/agent/task_handler.go` | 保证同一 conversation 的消息串行处理，避免上下文竞争 |
| 7.2 | Semaphore 并发控制 | `internal/agent/semaphore.go` | 限制同时处理的 Agent task 数量，防止 LLM API 过载 |
| 7.3 | 超时配置 | `internal/agent/task_handler.go` | 单次 LLM 调用超时、整体 task 超时 |
| 7.4 | `reload_agents` RPC | `internal/handler/` | 运行时热更新 Agent 配置，无需重启 server |
| 7.5 | LLM 调用监控 | `internal/agent/middleware_metrics.go` | Eino `WrapModel` middleware 记录延迟、token 用量、错误率 |
| 7.6 | 日志增强 | `internal/agent/task_handler.go` | 结构化日志：AgentID、ConversationID、耗时、token 数 |
| 7.7 | 缓存 TTL 清理 | `internal/agent/db_context_manager.go` | sync.Map 缓存过期清理，防止内存泄漏 |
| 7.8 | 压力测试 | | 并发发送多条消息，验证串行队列和 semaphore 工作正常 |

**产出**：生产级可靠性和可观测性。

**依赖**：Phase 5（核心编排）+ Phase 6（端到端验证通过后才有优化目标）

**验收标准**：
- 同一 conversation 的消息严格串行处理
- Semaphore 限制并发数，超出时排队等待
- `reload_agents` RPC 触发后新配置立即生效
- Langfuse / 日志中可追踪每次 LLM 调用的完整链路
- 缓存在 TTL 后自动清理
...
**目标**：扩展 Agent 能力边界，支持工具调用、子 Agent、HITL 等高级场景。

**任务**：

| # | 任务 | 文件 | 说明 |
| --- | ------ | ------ | ------ |
| 8.1 | 自定义 Tools 系统 | `internal/agent/tools/` | 基于 `utils.InferTool` 实现自定义工具（天气查询、数据库查询等） |
| 8.2 | 工具结果截取与检索 | `internal/agent/tools/retrieve.go` | 超长工具结果截取 + `retrieve_tool_result` 工具 |
| 8.3 | Sub-agents（DeepAgent） | `internal/agent/eino_agent.go` | 支持 Agent 配置中声明子 Agent，任务委派 |
| 8.4 | Middleware 集成 | `internal/agent/task_handler.go` | 集成 Summarization + ToolReduction + PatchToolCalls 中间件 |
| 8.5 | Human-in-the-Loop | `internal/agent/task_handler.go` | CheckpointStore（Redis）+ Interrupt/Resume 流程 |
| 8.6 | 新增 Agent Update 类型 | 协议层 | `agent_status`、`agent_question`、`agent_checkpoint_created`、`agent_timeout` |
| 8.7 | MCP 桥接 | `internal/agent/tools/mcp.go` | 将 MCP Server 工具转为 Eino Tool |
| 8.8 | Graph orchestration（可选） | `internal/agent/` | 复杂多步骤工作流，使用 Eino Graph |

**产出**：完整的 Agent 能力平台。

**依赖**：Phase 7（生产化基础）+ Phase 5 的 CheckPointStore 基础设施

**验收标准**：
- Agent 可调用自定义工具并获取结果
- 超长工具结果被截取，可通过 retrieve 工具获取完整内容
- Sub-agent 委派工作正常
- HITL 流程：Agent 提问 → 用户回答 → Agent 继续处理
- Summarization 中间件在长对话中自动触发压缩
...
| Phase | 依赖 | 可并行 | 预估工时 |
| ------- | ------ | -------- | --------- |
| **1. Agent 配置系统** | 无 | — | 2-3 天 |
| **2. 上下文管理** | P1 | P2 ∥ P3 ∥ P4 | 2-3 天 |
| **3. 消息路由与 MQ** | P1 | P2 ∥ P3 ∥ P4 | 1-2 天 |
| **4. Eino 封装与流式桥接** | P1 | P2 ∥ P3 ∥ P4 | 3-4 天 |
| **5. AgentTaskHandler 核心编排** | P1+P2+P3+P4 | — | 3-4 天 |
| **6. 客户端集成与端到端** | P5 | — | 2-3 天 |
| **7. 生产化加固** | P5+P6 | — | 3-5 天 |
| **8. 高级功能** | P7 | — | 5-7 天 |

**总计预估**：21-31 天（P2/P3/P4 并行可节省 3-5 天）
...
| 日期       | 版本 | 变更                                                                 |
| ---------- | ---- | -------------------------------------------------------------------- |
| 2026-07-11 | v2.1 | 添加 8-phase 实施阶段拆分与依赖分析                                  |
