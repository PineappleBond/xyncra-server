...
| 3.2 | 修改 `send_message` handler | `internal/handler/send_message.go` | 消息持久化后，检查对话对方的 UserID 是否有 `agent/` 前缀 |
| 3.3 | Agent 目标检测逻辑 | `internal/handler/send_message.go` | 获取 Conversation → 找到 peer UserID → `strings.HasPrefix(peer, "agent/")` → Enqueue |
...
| # | 任务 | 文件 | 说明 |
| --- | ------ | ------ | ------ |
| 6.1 | 实现 `isAgentUser()` helper | `pkg/client/agent.go` | `strings.HasPrefix(userID, "agent/")` 判断 |
