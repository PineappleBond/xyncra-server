...
| D-113 | interruptIDs 内存存储策略 | sync.Map 存储在 AgentExecutor，daemon 重启丢失，与 D-080 一致 |
...
| `interrupt_id` | 否 | Eino 框架定位中断点。客户端可从 `agent_question` 事件获取，用于精确 resume。如果未提供，服务端从内存 `interruptIDs` map 中查找 |
| `answer` | 是 | 用户对 agent 问题的回答 |
| `agent_id` | 是 | 多 Agent 场景标识 |

Handler 入队 `TypeAgentResume` MQ 任务。任务处理器加载 checkpoint，注入用户答案到 Agent 上下文，恢复执行。遵循 D-073（总是返回 nil 给 MQ）和 D-002（无鉴权）。
...
3. **与 D-050 互补**：ReverseRPC 不替代 ephemeral push，而是补充。HITL 中的 `agent_question` ephemeral 通知仍保留（用于 UI 展示），ReverseRPC 用于结构化地获取用户回答
...
## D-113: interruptIDs 内存存储策略

### 决策

使用 `sync.Map` 存储在 `AgentExecutor` 中，映射 `conversationID → interruptID`。daemon 重启后丢失，不持久化。与 D-080（tool result truncation）策略一致：瞬时数据无需持久化。resume 成功后清理对应 key，防止内存泄漏。

### 原因

1. **与 D-080 一致**：瞬时数据使用 `sync.Map` + 内存存储是已有模式
2. **无需持久化**：interruptID 仅在单次 HITL 流程中有意义，daemon 重启后 HITL 流程已中断，interruptID 无价值
3. **防止内存泄漏**：resume 成功后立即删除对应 key
4. **fallback 机制**：客户端可通过 `agent_question` 事件获取 interrupt_id 并传递给 `agent_resume`（D-085），即使内存 map 中丢失也能 resume

### 约束

- key 为 `conversationID`，value 为 `interruptID`
- daemon 重启后 map 清空，客户端需重新触发 HITL 流程或自行提供 interrupt_id
- resume 成功后必须清理 key，避免长期堆积
...
- daemon 默认 `device-name="xyncra-cli"`, `device-type="cli"`，可通过 `listen --device-name` / `listen --device-type` 覆盖
