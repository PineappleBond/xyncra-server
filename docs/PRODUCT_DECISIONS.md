...
同 `(userID, deviceID)` 新连接到来时，向旧连接发送 Close Frame（code: 4001, reason: "replaced by new connection from same device"），旧连接立即关闭。旧连接的 pending ReverseRPC 请求通过 `CancelDevice(userID, deviceID)` 立即 fail（不等超时）。新连接在旧连接清理后注册。
...
- 替换时序：先关闭旧连接 → 等待旧连接清理（最多 2s）→ 注册新连接
