...
export class XyncraClient {
  ...
  private async reregisterFunctions(): Promise<void> {
    ...
    try {
      ...
      await withTimeout(this.call('system.register_functions', params), 10_000);
      this.logger.info('Functions registered', { count: fns.length });
    } catch (error) {
      this.logger.error('Function registration failed (fail-open)', {
        error,
        count: fns.length,
      });
    }
