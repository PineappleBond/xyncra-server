...
退出码：`2`（D-042 -- precondition not met）。
...
```
Agent resume queued successfully
...
```
Error: agent-resume: checkpoint expired
```
...
| 场景 | 错误信息 | 原因 | 解决方法 |
|------|---------|------|---------|
| daemon 未运行 | `Error: daemon not running.` | daemon 未启动 | 先运行 `xyncra-client listen` |
| checkpoint 过期 | `agent-resume: checkpoint expired` | Checkpoint TTL（24h）已过期 | 重新发送消息触发新的 HITL |
| interrupt_id 不匹配 | `agent-resume: interrupt not found` | 使用了旧的 interrupt_id | 使用最新 `[hitl]` 输出中的 ID |
| 参数缺失 | `error: missing required argument '--checkpoint-id'` | 缺少必需参数 | 检查命令参数是否完整 |
...
- **IPC-only** (D-036, D-114): The daemon forwards the `agent_resume` request to the server via its persistent WebSocket connection. No standalone WebSocket fallback is provided.
