...
- **Why no fallback?** (D-036): The daemon holds `localMaxSeq` and a deduplication state (NotificationLog). An independent WebSocket calling `sync_updates` RPC directly would conflict with the daemon's IndexedDB writes and potentially cause duplicate update processing.
