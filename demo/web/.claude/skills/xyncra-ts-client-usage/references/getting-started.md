...
本文档帮助你在 5 分钟内跑通 `@xyncra/client-cli` 的核心流程：构建、启动守护进程、发送消息、查询数据。
...
# 全局链接 CLI（使 xyncra-client 命令可用）
npm link
```

> **注意**：客户端使用 npm workspace 管理 3 个包：
> - `@xyncra/protocol` — 协议类型定义
> - `@xyncra/client-core` — 环境无关核心（DI 接口、Dexie DB、连接管理、同步管理）
> - `@xyncra/client-cli` — Node.js CLI 运行时（Commander CLI、daemon、IPC）
>
> 依赖链：`client-cli` → `client-core` → `protocol`
...
> - 同一 (user_id, device_id) 只允许一个实例，由 fs-ext 进程锁保证
> - 退出码 `2` 表示锁冲突（进程已在运行）
...
查询命令通过 IPC 连接 daemon 读取本地 Dexie.js 数据库（**TS-D-002**），需要守护进程运行。
...
> **注意**：与 Go 客户端不同，TS 客户端的查询命令通过 IPC 访问 daemon（因为 Dexie.js 运行在 daemon 进程中），不支持直接离线查询。数据反映最后一次同步时的状态。
...
| `XYNCRA_DEVICE_ID` | `--device-id` | `SHA256(hostname)[:8]` | 设备 ID（**D-033**） |
| `XYNCRA_SERVER` | `--server, -s` | `ws://localhost:8080/ws` | 服务器 URL |
| `XYNCRA_DB_PATH` | `--db-path` | `$USER_DIR/xyncra.db` | IndexedDB 数据库名（**TS-D-012**） |
| `XYNCRA_LOG_DIR` | `--log-dir` | `$USER_DIR/logs` | 日志目录 |
...
`--device-id` 默认值为 `SHA256(hostname)[:8]`。如果省略，客户端会自动从主机名派生设备标识。
可通过 `--device-id` flag 或 `XYNCRA_DEVICE_ID` 环境变量覆盖默认值。
...
├── xyncra.lock     # 进程锁文件（fs-ext，D-031）
...
| `xyncra.lock` | - | fs-ext 文件锁（**D-031**），支持 stale lock 检测 |
...
- [架构概览](architecture/overview.md) — 系统架构和数据流
- [Dexie.js 数据库结构](architecture/database.md) — 9 个表的详细说明
- [IPC 协议规范](architecture/ipc.md) — JSON-RPC 2.0 over Unix Socket
