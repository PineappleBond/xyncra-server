...
This document explains how the `xyncra-client` TypeScript CLI synchronizes data between the server and local IndexedDB, and how it handles various connectivity conditions.
...
2. **Local Tracking (`localMaxSeq`)**: The daemon maintains a `localMaxSeq` value in the local IndexedDB (via Dexie.js), recording the highest `seq` it has processed. This is the synchronization cursor.
...
|------|-------------|---------------|
| `message` | New message | Persist to `messages` table in IndexedDB |
...
3. The daemon receives the update, persists the message to IndexedDB, and advances `localMaxSeq`
...
Query commands read from the daemon's IndexedDB via IPC and immediately reflect the latest synced state:
...
> **Note**: Unlike the Go client, query commands cannot read a local SQLite file directly. They must go through IPC to the daemon process because IndexedDB lives in the daemon's memory (TS-D-012). The daemon must be running.
...
3. The local IndexedDB may have stale data that cannot be incrementally repaired
...
3. If data still appears incomplete, the gap is likely due to server-side cleanup (D-016). The local IndexedDB reflects whatever the server could provide.
...
> **TS-specific note**: Since IndexedDB is in-process memory (fake-indexeddb in Node.js), restarting the daemon loses all in-memory data. The daemon must perform a FullSync on each startup to rebuild its local state. This differs from the Go client where SQLite persists across daemon restarts.
...
1. **State consistency**: The daemon holds `localMaxSeq` and the WebSocket connection. An independent WebSocket connection calling `sync_updates` directly would compete with the daemon for IndexedDB writes.
