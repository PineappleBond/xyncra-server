...
This document covers how the `xyncra-client` TypeScript CLI handles multiple devices, instances, and data isolation.
...
- Its own IndexedDB instance (in daemon process memory via fake-indexeddb, TS-D-012)
- Its own IPC socket (`xyncra.sock`)
- Its own lock file (`xyncra.lock`, managed by fs-ext)
...
> **Note**: Unlike the Go client, there is no `xyncra.db` SQLite file on disk. The TS client uses IndexedDB (via `fake-indexeddb` in Node.js), which lives in the daemon process memory.
...
| IndexedDB | `device1` (in-memory) | `device2` (in-memory) |
| Socket | `device1/xyncra.sock` | `device2/xyncra.sock` |
| Lock | `device1/xyncra.lock` | `device2/xyncra.lock` |
| Logs | `device1/logs/` | `device2/logs/` |

Querying from a specific instance (all queries go through IPC to the daemon, TS-D-012):

```bash
# Read data from device1's IndexedDB via IPC
xyncra-client list-conversations --user-id alice --device-id device1

# Read data from device2's IndexedDB via IPC
...
Both receive `UserUpdate` pushes (D-028). Each daemon independently processes updates and persists to its own IndexedDB instance.
...
> **Note**: No `xyncra.db` file exists. The TS client uses IndexedDB (Dexie.js) in the daemon process memory (TS-D-012).

### File Permissions

- Directories: `0700` (owner-only access)
- Socket files: `0600` (owner-only read/write)
- Lock files: managed by `fs-ext` (kernel-level on supported platforms, D-031)

### Database Isolation

Each instance's IndexedDB database is isolated by device ID (the `--db-path` value is used as the Dexie database name, TS-D-012). Since `fake-indexeddb` operates in process memory, each daemon process has its own completely separate data store.
