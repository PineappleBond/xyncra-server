...
defineTestHelpers('dashboard-analysis', {
  ...
  switchDateRange: {
    ...
    parameters: {
      type: 'object',
      properties: {
        range: { type: 'string', description: '时间范围（today/week/month/year）' }
      },
      required: ['range']
    },
        ...
        <el-card shadow="never">
          <template #header>
            <div class="flex justify-between items-center">
              ...
              <el-segmented v-model="activeMetric" :options="metrics.map(m => ({ value: m.key, label: m.label }))" @change="onMetricChange" />
            </div>
          </template>
          <div class="text-3xl font-bold">{{ activeMetric === 'sales' ? '¥ 126,560' : '8,846' }}</div>
      ...
      <el-col :span="12">
        <el-card shadow="never">
          ...
          <el-radio-group v-model="activeDateRange" @change="onDateRangeChange">
            <el-radio-button v-for="range in dateRanges" :key="range.key" :value="range.key">
              {{ range.label }}
            </el-radio-button>
          </el-radio-group>
        </el-card>
