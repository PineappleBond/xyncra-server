...
3. 使用 codegraph 或 Read 深入探索与任务直接相关的代码
...
1. **运行测试** — 执行 `go test ./...`，记录所有失败
2. **多角度 Review** — 串行调度以下子代理审查当前代码：
   - **后端架构师**：检查架构合理性、接口一致性、错误处理、doc.go & *.md文档落后、完整性
...
3. **Git 状态检查**
   - 检查 `.gitignore` 是否需要更新（新类型的文件是否需要忽略）
   - 确认没有意外提交的文件（如 `.env`、临时数据库文件、IDE 配置）
   - `git status` 查看所有变更
...
5. **提交与推送**
   ...
   - scope: 受影响的模块（如 server, store, mq, protocol）
   - 示例：`feat(server): implement send_message method handler`
   - `git add` 相关变更
   - `git commit` 提交
   - `git push` 推送到远程
