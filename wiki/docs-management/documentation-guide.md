...
技术术语（面向前端开发者，但需要更好的受众细分，）使用英文原文，首次出现时在括号内标注中文翻译：
...
| 纯文档改进（无代码变更） | 不需要版本号，只需在 Front Matter 更新 `updated` 日期 |
| 文档重组（移动、重命名） | 在 CHANGELOG 或 PR 描述中记录 |

### Front Matter 日期管理

每次对文档进行实质性修改后，更新 Front Matter 中的 `updated` 字段：
