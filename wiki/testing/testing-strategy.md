...
```
        ┌──────────┐
        │  E2E     │  73 个测试（72 PASS / 1 FAIL）
        │  集成    │  覆盖：消息投递、Agent 全流程、HITL、断连重连
       ┌┴──────────┴┐
       │  集成测试   │  46 个 Server E2E + 8 个 CLI E2E
       │  (需要 Redis)│  覆盖：HTTP 路由、Redis 连接存储、MQ 消息队列
      ┌┴────────────┴┐
      │  单元测试     │  818+ 个测试，3 个包有独立 TESTING_USAGE.md
...
| 包 | 目标覆盖率 | 当前状态 | 关键路径 |
|----|-----------|---------|---------|
| `internal/server` | >80% | 226+ 测试 | WebSocket 协议、连接管理 |
| `internal/mq` | >80% | 20+ 测试 | 消息路由、任务入队 |
| `internal/store` | >90% | 11 测试函数 x 3 数据库 | CRUD、事务、迁移 |
| `internal/agent` | >75% | 60+ E2E + 单元 | 执行器、HITL、子代理 |
