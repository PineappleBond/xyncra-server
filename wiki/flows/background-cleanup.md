...
本文档描述 Xyncra 服务器和服务端/客户端中的后台清理任务，包括 UserUpdate 过期清理、HITL 超时清理、上下文缓存清理、工具结果清理、Rate Limiter 清理和客户端日志清理。大部分服务端清理任务在 `cmd/xyncra-server/main.go` 中直接启动，通过共享的 `context.Context` 管理生命周期；Rate Limiter 清理任务则在各自的 handler 构造函数中启动（通过 `RegisterAll` 间接调用），使用 `for range ticker.C` 模式（不响应 `ctx.Done()`，依赖进程退出终止）。
...
| 工具结果清理 | 5 分钟 | 1 小时 | 清理过期的工具执行结果（TTL 可配置） |
...
3. **SQL 操作**：`DELETE FROM user_updates WHERE created_at < NOW() - INTERVAL 30 DAY`（GORM `Unscoped()` 确保硬删除，绕过软删除机制）
4. **记录日志**：如果删除行数 > 0，记录日志
5. **错误处理**：清理失败仅记录日志，不中断循环
6. **生命周期**：通过 `ctx.Done()` 响应取消信号，优雅退出
        ...
        loop 每个超时会话
            ...
            alt 获取锁失败
                ...
                alt 状态已不是 asking_user
                    ...
                    CS->>DB: UPDATE conversations SET agent_status='idle', agent_id='', checkpoint_id='', agent_last_activity=now, updated_at=now
                    DB-->>CS: 返回 updated_at
...
1. **定时触发**：由调用方通过 `StartCleanup(ctx, interval)` 启动，服务端在 `main.go` 中以 5 分钟间隔启动（`DefaultToolResultStore.StartCleanup(ctx, 5*time.Minute)`）。通过 `ctx.Done()` 响应取消信号，优雅退出
...
1. **定时触发**：每 5 分钟触发一次（两个独立 goroutine，分别在 `NewStreamTextHandler` 和 `NewSetTypingHandler` 构造函数中通过 `go h.cleanupLoop()` 启动，由 `RegisterAll` 间接调用）
2. **遍历条目**：遍历 `sync.Map` 中的所有条目
3. **检查访问时间**：获取每个条目的 `lastAccess` 时间（加锁读取后立即释放）
4. **删除过期条目**：删除 10 分钟未访问的条目
5. **注意**：`cleanupLoop` 使用 `for range ticker.C` 无限循环，没有 `select` 监听 `ctx.Done()`，也没有 panic recovery。goroutine 依赖进程退出终止，不响应取消信号。如果 `cleanupStaleLimiters` panic，goroutine 将终止，rate limiter 条目将不再被清理
...
| context 取消 | **不响应 ctx.Done()**（`for range ticker.C` 无 select），goroutine 持续运行直到进程退出 |
